    ...
    function callExecute(ele) {
        ...
        if (attrObj.addtomodel) {
            ...
            for (var prop in newAdd) {
                if (scope[prop]) {
                    if (scope[prop] instanceof Object && newAdd[prop] instanceof Object) {
                        scope[prop] = extend(true, scope[prop], newAdd[prop]);
...
exports.getfromangular = getfromangular = function getfromangular(parameters, callback) {
    if ($ && $('body').scope() !== undefined) { callback(null, $('body').scope()[parameters.wid]); }
    else { callback(null); }
};
