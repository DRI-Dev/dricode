...
(function (window) {
    ...
    exports.getdtoobject = getdtoobject = function getdtoobject(obj, command, callback) {
                    ...
                    if (inobj.hasOwnProperty(eachparm)) {
                        ...
                        if (isObject(inobj[eachparm]) || isArray(inobj[eachparm])) {
                            ...
                            if (dtotable[eachparm]) { // if table entry exists, then merge to what you just got
                                ...
                                var potentialdto = {}
                                extend(true, potentialdto, dtotable[eachparm])

                                var relationshiptype = "";
                                if(potentialdto &&  potentialdto['metadata'] && potentialdto['metadata'][eachparm]){
                                    relationshiptype = potentialdto['metadata'][eachparm];
                                }
                         
                                var converteddto={}
                                if ((relationshiptype === "onetomany") || (relationshiptype === "manytomany")  || (relationshiptype === "jsononetomany") || (relationshiptype === "jsonmanytomany")) {
                                    
                                    if(!(potentialdto instanceof Array)) {
                                        // shall be converted to Array
                                        converteddto=[];
                                        converteddto.push(potentialdto);
                                    }else{
                                        converteddto=potentialdto;
                                    }
                                }else{
                                    converteddto=potentialdto;
                                }

                                // use converted dto below
                                if (isArray(converteddto)) { // get a object copy of dtotable[eachparam] to tempobj
                                    tempobj = converteddto[0];
                                } else {
                                    tempobj = converteddto;
                                }
        ...
        // if (obj.metadata.method === "string" ||  obj.wid ==="string") {
        //     callback(null, obj)
        // }else{
            

            if (dtotype === "defaultdto" || obj.metadata.method === obj.wid) {
                ...
                console.log("getdtoobject about to getwidmaster  " + JSON.stringify(obj)); //38
                execute({
                    "executethis": "getwidmaster",
                    "wid": dtotype,
                    "command.getwidmaster.convertmethod": "dto",
